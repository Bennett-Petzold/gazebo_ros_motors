...
void GazeboRosMotor::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  gazebo_ros_->getParameter<double>(
      motor_nominal_voltage_, "motor_nominal_voltage", 24.0); // Datasheet 24.0V
...
bool GazeboRosMotor::checkParameters() {
  if (this->armature_damping_ratio_ != 0.0 &&
      !isnan(this->armature_damping_ratio_) &&
      ...
      !isnan(this->electromotive_force_constant_) &&
...
void GazeboRosMotor::publishEncoderCount(double m_vel, double dT) {
  ...
  encoder_counter_ +=
      ((rev_in_rad) / 2 * M_PI) * encoder_pulses_per_revolution_;
      ...
      (emA *
       (i0 * (Km2 + d * R) *
        ...
        J * (Km3 * (-2 * eOm1 * o0 * Om + 4 * (-2 * eA + eOp1) * L * T) -
             Km * R *
                 (2 * d * eOm1 * o0 * Om - 2 * d * (-2 * eA + eOp1) * L * T +
                  eOm1 * Om * T) +
  ...
  double o_t =
      (emA *
       (-4 * eOp1 * J * pow(Km, 4) * L * o0 +
        J * Km2 * R *
            (-6 * d * eOp1 * L * o0 + eOm1 * o0 * Om -
             4 * (-2 * eA + eOp1) * L * T) +
        J * R2 *
            (-2 * d2 * eOp1 * L * o0 + d * eOm1 * o0 * Om -
             2 * d * (-2 * eA + eOp1) * L * T + eOm1 * Om * T) +
        4 * (-2 * eA + eOp1) * J * Km3 * L * V -
        J * Km * (-2 * d * (-2 * eA + eOp1) * L + eOm1 * Om) * R * V +
        J2 * R2 *
            (eOp1 * Km2 * o0 + d * eOp1 * o0 * R + (-2 * eA + eOp1) * R * T -
             (-2 * eA + eOp1) * Km * V) +
